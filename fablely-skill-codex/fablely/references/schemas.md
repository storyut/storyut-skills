...
Rules:
- Rewrite in place — this file has no history, it only describes now; delete what no longer describes now.
- When a work unit completes, collapse its Current state entry to one line: number, outcome, and a pointer to the work file's Outcome.
...
Rules:
- Every clause carries a stable ID (`SEC-1`, `ERR-2`, `PRJ-3`), cited in work units' Clears-the-bar blocks, in Small Intents' `Done when`, and in review findings. Never renumber an existing ID — retire it into Not applicable instead, or citations elsewhere in `.fable/` start pointing at the wrong rule.
...
- Adding a project-specific clause is cheap. Promote a `LESSONS.md` entry here once the same mistake has recurred — a lesson records one past failure, a standard guards against the next one.
...
What actually happened and what evidence was observed.
```

Rules: `NNN` is the next unused zero-padded number starting from `001`. The standards box gates the rest: status does not flip to `done` and no checkpoint commit happens while a standards finding is open and unwaived. Design, spec, and plan stay in one file. Done work files remain historical; resume reads PROGRESS and follows its Outcome pointer only when detail is needed.
...
Rules: one file per investigation; `slug` is short kebab-case. Primary sources only — official docs, source code, specs; a claim without a source it can be checked against doesn't belong in the file. Research files are reference material, not memory: they record what was true at `date`, and a work file citing one inherits that date.

## Not project memory

`.fable/.intent` is the per-task intent stamp (gitignored). It is not project memory — don't read it for context, don't write project state into it.
