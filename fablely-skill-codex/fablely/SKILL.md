---
name: fablely-codex
description: Codex port of the fablely project-continuity workflow backed by `.fable/`. Use when `.fable/PROGRESS.md` exists, AGENTS.md points to it, the user asks to bootstrap/adopt fablely, or state must be recovered after interruption or compaction. Never auto-adopt an ordinary repository or a directory merely containing a fablely artifact.
---
...
User instructions, system rules, permissions, AGENTS.md hierarchy, and repository rules outrank this skill. Fablely coordinates authorized work; it grants no authority to scaffold, mutate git, delegate, or exceed path scope.

## 1. Activate

| Observed state | Mode | Action |
|---|---|---|
| `.fable/PROGRESS.md` exists or AGENTS.md points to it | **Resume** | Run the resume protocol before state-dependent work. |
...
| Otherwise | **Inactive** | Continue normally; create no fablely, AGENTS.md, or git state. |
...
| Lane | Test | Protocol |
|---|---|---|
| **Consult** | Answer/explain/investigate; no edits | Answer read-only inline. `/fablely-ask-codex` is the user-invoked shortcut, not an automatic dependency. Drift-check only state-dependent claims. |
| **Diagnose** | Broken, failing, throwing, or slow | Use `fablely-debug-codex` read-only first. One `small: diagnose <symptom>` intent stands through an authorized fix. |
...
| **Medium+** | Several plausible shapes or hard to undo: public interface, multiple components, security/data risk, migration | Use `fablely-spec-codex` for one `.fable/work/NNN-slug.md`, then execute it. |
...
Bootstrap/Adopt use the approved proposal as pre-write intent. An allowlist excluding `.fable/` forbids intent and memory writes. Keep fablely context read-only, perform only otherwise-authorized work inside the allowlist, and report that continuity could not be recorded.
...
Use `templates/`. Before writing, present the exact scaffold, AGENTS.md and `.gitignore` changes, plus each git action; then **CHECKPOINT · STOP**. Approval covers only listed items.

- **Bootstrap:** derive MISSION; present `STANDARDS.md` unchanged and ask which clauses do not apply; after approval create selected files and empty `.fable/work/`. Do not instantiate `work-unit.md` or overwrite existing settings/AGENTS.md.
- **Adopt:** inspect code, docs, standards, linter config, and history read-only. Draft MISSION, MAP, DECISIONS, STANDARDS, AGENTS.md contract, and `.gitignore`. Preserve documented conventions, add stable `PRJ-` clauses, and report existing breaches instead of lowering the bar. If declined, write nothing and continue inactive. Never initialize git unless separately requested.

Append the contract to an existing AGENTS.md; never replace unrelated instructions.
...
- Missing dependency/tool: use an in-scope equivalent; stop if it weakens acceptance.
...
- `templates/AGENTS.md.template`, `templates/fable/` — scaffolds
- User-invoked `fablely-ask-codex` — strict Consult shortcut; `fablely-debug-codex` — diagnosis; `fablely-spec-codex` — Medium+ design
