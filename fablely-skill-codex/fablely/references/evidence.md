...
Relevant tests and assertions passed, but you did not establish causality or observe the complete behavior. This supports a narrow claim about those checks—not a claim about every path through the feature.

> The focused rounding tests passed; I did not exercise the checkout flow end-to-end.
...
A hypothesis or recollection. Inspect before relying on it and label it plainly if it must be reported.

## Consequences

- **Green is bounded evidence.** A passing test proves its assertions passed in that run. It does not by itself prove the change caused the result or that unasserted behavior works.
- **Read inherited tests before relying on them.** Prefer a focused red/green regression test when practical, but never damage user code merely to manufacture a red phase.
