...
A request to diagnose or debug authorizes read-only investigation and existing commands, not persistent changes. Creating a harness, adding instrumentation or a regression test, and applying the fix require explicit mutation authority. If the request does not include it, carry the diagnosis as far as existing evidence permits, then stop with the confirmed cause, evidence, and prospective fix.
