...
| [`fablely-ask`](fablely-skill/fablely-ask/SKILL.md) | 用户主动调用的咨询入口。回答关于项目的问题，什么都不碰：不盖意图戳，不写 PROGRESS，不改文件。 |
...
不一样的只有非不一样不可的部分。两版现在都按同一套精简原则整理；Codex 版保留适合 GPT-5.6 的结论先行和决策规则，Claude Code 版则保留 hook 专用指令。Codex 没有生命周期 hook，所以在 Claude Code 里靠 hook 兜住的保证，这边改由 `AGENTS.md` 的正文承担。移植的是行为，不是措辞。差异都是有意为之，也[写在文档里](fablely-skill-codex/README.md)。
